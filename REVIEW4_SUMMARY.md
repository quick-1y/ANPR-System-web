...
| 2 | Monolithic app.js (3138 lines, no modules) 🚧 In progress | High | Maintenance bottleneck, untestable; Task 15 steps 1-11 done (`api.js` for API/auth/fetch + `state.js` for shared state/defaults + `debug.js` for debug/logging + `journal.js` for journal/history + `lists.js` for plate lists/import-export + `settings.js` for settings load/save + `controllers.js` for controllers CRUD/test/hotkeys + `ui.js` for shared UI helpers + `help.js` for shared help popover helpers + `events.js` for live events/feed widgets + `backup.js` for backup/restore flows), but full split is still pending |
