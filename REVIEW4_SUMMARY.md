...
| 2 | Monolithic app.js (3138 lines, no modules) ⏳ In progress since 2026-03-30 (step 1: API/auth extracted to `app/web/js/api.js`; step 2: state extracted to `app/web/js/state.js`; step 3: channels extracted to `app/web/js/channels.js`) | High | Maintenance bottleneck, untestable |
...
- Completed incremental sub-steps:
  ...
  - Step 3: extracted channel module into `app/web/js/channels.js` (channel grid, preview/overlays, ROI editor, plate-size editor, channel CRUD).
