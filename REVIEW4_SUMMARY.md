...
| 2 | Monolithic app.js (3138 lines, no modules) 🚧 In progress | High | Maintenance bottleneck, untestable; Task 15 steps 1-4 done (`api.js` for API/auth/fetch + `state.js` for shared state/defaults + `debug.js` for debug/logging + `journal.js` for journal/history), but full split is still pending |
