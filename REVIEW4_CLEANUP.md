...
| 8 | Monolithic `app.js` (3138 lines) 🚧 In progress (Task 15, steps 1-4 done) | `app/web/app.js`, `app/web/js/api.js`, `app/web/js/state.js`, `app/web/js/debug.js`, `app/web/js/journal.js`, `app/web/index.html` | Started module split: step 1 extracted API/auth/fetch to `api.js`; step 2 extracted shared app state/defaults to `state.js`; step 3 extracted debug/logging panel+stream logic to `debug.js`; step 4 extracted journal/event history loading+filters+details to `journal.js`; `app.js` orchestrates module calls |
