...
def _make_container(
    ...
    container.user_db.count_active_admins.return_value = active_admin_count
...
_ADMIN = _make_user(user_id=1, login="admin", role="admin")
...
class TestListUsers:
    def test_admin_gets_all_users(self):
        container = _make_container(users=[_ADMIN, _OPERATOR])
        result = list_users(current_user=_ADMIN, container=container)
        assert len(result) == 2
        logins = {u.login for u in result}
        assert logins == {"admin", "op1"}

    def test_passwords_not_exposed(self):
        container = _make_container(users=[_ADMIN])
        result = list_users(current_user=_ADMIN, container=container)
...
class TestCreateUser:
    def test_creates_new_user(self):
        ...
        result = create_user(body=body, current_user=_ADMIN, container=container)
    ...
    def test_duplicate_login_returns_409(self):
        ...
        with pytest.raises(HTTPException) as exc:
            create_user(body=body, current_user=_ADMIN, container=container)
...
class TestGetUser:
    def test_returns_user_by_id(self):
        container = _make_container(users=[_ADMIN, _OPERATOR])
        result = get_user(user_id=2, current_user=_ADMIN, container=container)
        assert result.login == "op1"

    def test_not_found_returns_404(self):
        container = _make_container(users=[_ADMIN])
        with pytest.raises(HTTPException) as exc:
            get_user(user_id=999, current_user=_ADMIN, container=container)
...
class TestUpdateUser:
    def test_update_role(self):
        users = [_make_user(user_id=1, role="admin"), _make_user(user_id=2, login="op1", role="operator")]
    ...
    def test_update_permissions(self):
        ...
        result = update_user(user_id=2, body=body, current_user=_ADMIN, container=container)
    ...
    def test_not_found_returns_404(self):
        ...
        with pytest.raises(HTTPException) as exc:
            update_user(user_id=999, body=body, current_user=_ADMIN, container=container)
        assert exc.value.status_code == 404
    ...
    def test_self_deactivation_blocked(self):
        ...
        with pytest.raises(HTTPException) as exc:
            update_user(user_id=1, body=body, current_user=_ADMIN, container=container)
        assert exc.value.status_code == 400

    def test_last_admin_role_removal_blocked(self):
        """Removing admin role from yourself when you're the only active admin must fail."""
        container = _make_container(users=[_ADMIN], active_admin_count=1)
        body = UserUpdate(role="operator")
        with pytest.raises(HTTPException) as exc:
            update_user(user_id=1, body=body, current_user=_ADMIN, container=container)
        assert exc.value.status_code == 400

    def test_admin_role_removal_allowed_when_multiple_admins(self):
        """Can remove admin role from self when there are other active admins."""
        admin2 = _make_user(user_id=3, login="admin2", role="admin")
        users = [_ADMIN, admin2]
        container = _make_container(users=users, active_admin_count=2)
        container.user_db.update_user.return_value = {**_ADMIN, "role": "operator"}
        body = UserUpdate(role="operator")
        result = update_user(user_id=1, body=body, current_user=_ADMIN, container=container)
        assert result.role == "operator"
...
class TestChangePassword:
    def test_admin_can_change_any_password(self):
        ...
        result = change_password(user_id=2, body=body, current_user=_ADMIN, container=container)
    ...
    def test_not_found_returns_404(self):
        ...
        body = UserPasswordChange(new_password="newpass1")
        with pytest.raises(HTTPException) as exc:
            change_password(user_id=999, body=body, current_user=_ADMIN, container=container)
        assert exc.value.status_code == 404
...
class TestDeactivateUser:
    def test_admin_can_deactivate_other(self):
        container = _make_container(users=[_ADMIN, _OPERATOR])
        # Should not raise
        deactivate_user(user_id=2, current_user=_ADMIN, container=container)
        container.user_db.deactivate.assert_called_once_with(2)

    def test_self_deactivation_blocked(self):
        container = _make_container(users=[_ADMIN])
        with pytest.raises(HTTPException) as exc:
            deactivate_user(user_id=1, current_user=_ADMIN, container=container)
        assert exc.value.status_code == 400

    def test_not_found_returns_404(self):
        container = _make_container(users=[_ADMIN])
        with pytest.raises(HTTPException) as exc:
            deactivate_user(user_id=999, current_user=_ADMIN, container=container)
        assert exc.value.status_code == 404
