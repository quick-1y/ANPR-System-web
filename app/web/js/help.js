...
const PARAM_HELP = {
  ...
  g_style: "Визуальный стиль приложения. Graphite Minimal используется по умолчанию; Aurora — отдельный современный стиль с собственным визуальным слоем и светлой/тёмной темой.",
  g_theme: "Светлая или тёмная тема внутри выбранного стиля приложения. Тёмная тема снижает нагрузку на глаза при работе в условиях слабого освещения.",
  g_token_ttl: "Сколько минут действует сессия после входа (5–43200, по умолчанию 480 = 8 часов). Изменение применяется к следующим входам; уже выданные сессии живут до своего срока.",
  g_rl_attempts: "Сколько неудачных попыток входа допускается с одного IP-адреса за окно (1–100, по умолчанию 5). После превышения вход блокируется до конца окна. Применяется сразу.",
  g_rl_window: "Длина скользящего окна подсчёта неудачных попыток, секунд (1–86400, по умолчанию 60). Применяется сразу.",
  ...
  g_timezone: "Зона отображения времени инстанса (IANA, с учётом перехода на летнее время). Влияет на представление времени в интерфейсе и на время в экспорте CSV/ZIP; хранение остаётся в UTC, системные часы сервера не меняются.",
