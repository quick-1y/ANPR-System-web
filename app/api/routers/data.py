...
def get_data_policy(container: AppContainer = Depends(get_container), _user: Dict[str, Any] = Depends(get_current_user)) -> Dict[str, Any]:
    return container.lifecycle.policy.to_storage()


@router.put("/api/data/policy")
def update_data_policy(payload: RetentionPolicyPayload, container: AppContainer = Depends(get_container), _user: Dict[str, Any] = Depends(get_current_user)) -> Dict[str, Any]:
...
def run_retention(container: AppContainer = Depends(get_container), _user: Dict[str, Any] = Depends(get_current_user)) -> Dict[str, Any]:
    try:
...
def export_events_bundle(payload: ExportBundlePayload, container: AppContainer = Depends(get_container), _user: Dict[str, Any] = Depends(get_current_user)) -> Response:
    try:
...
def backup_database(container: AppContainer = Depends(get_container), _user: Dict[str, Any] = Depends(get_current_user)) -> Response:
...
def backup_settings(container: AppContainer = Depends(get_container), _user: Dict[str, Any] = Depends(get_current_user)) -> Response:
    try:
