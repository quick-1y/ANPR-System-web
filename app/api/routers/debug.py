...
from app.api.deps import get_container, require_superadmin
...
def get_debug_settings(container: AppContainer = Depends(get_container), _user: Dict[str, Any] = Depends(require_superadmin())) -> Dict[str, Any]:
    return container.processor.get_debug_settings()


@router.put("/api/debug/settings")
def put_debug_settings(payload: DebugPayload, container: AppContainer = Depends(get_container), _user: Dict[str, Any] = Depends(require_superadmin())) -> Dict[str, Any]:
...
def debug_channels(container: AppContainer = Depends(get_container), _user: Dict[str, Any] = Depends(require_superadmin())) -> Dict[str, Any]:
...
def debug_state(container: AppContainer = Depends(get_container), _user: Dict[str, Any] = Depends(require_superadmin())) -> Dict[str, Any]:
    return {
        "settings": container.processor.get_debug_settings(),
...
def debug_logs(limit: int = 200, container: AppContainer = Depends(get_container), _user: Dict[str, Any] = Depends(require_superadmin())) -> Dict[str, Any]:
    return {"items": container.debug_log_bus.snapshot(limit=limit)}


@router.get("/api/debug/logs/stream")
async def stream_debug_logs(request: Request, last_id: int = 0, container: AppContainer = Depends(get_container), _user: Dict[str, Any] = Depends(require_superadmin())) -> StreamingResponse:
    async def generator():
