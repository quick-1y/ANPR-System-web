...
│   │   ├── auth_utils.py       # JWT issue/verify, bcrypt password hashing
...
## Development And Rollback Policy

This project is under **active development**. The following is a deliberate project policy, not an oversight. It governs the rules in "Data, Contracts, Codegen, And Migrations" and "Database Rules" below.

### Policy

- **The current version is developed as a fresh implementation.** Supporting upgrades from arbitrary older development versions is not a requirement.
- **Do not introduce legacy migration modules, compatibility shims, or transitional upgrade paths whose only purpose is to support previous development versions.** This applies to database schema, settings/configuration, and API contracts alike.
- **Rollback is performed by reverting the entire project to a known-good Git revision** and recreating containers — not by running a reverse migration. Keep the working tree clean so this remains predictable.
- **Prefer a clean implementation over accumulating compatibility code.** When an architectural change is needed, change the architecture directly instead of layering adapters over the old one.
- **A clean initialization/bootstrap step is allowed and expected.** Idempotent schema creation (`CREATE TABLE IF NOT EXISTS`, `ALTER TABLE ... ADD COLUMN IF NOT EXISTS`) and defaults applied from code are part of defining the current schema — they are not version-migration machinery.

### What this policy does NOT mean

- It does **not** license careless or destructive changes. Irreversible steps (dropping a table, removing a settings layer, deleting an endpoint) still need to be isolated in their own PR, reviewed on their own, and validated on a real deployment first.
- It does **not** waive data safety. Ask before any change that can destroy operational data (events, media, users, lists).
- It does **not** forbid compatibility work outright. Introduce it when there is a **specific architectural reason** — for example an externally consumed API contract, or a genuine production deployment that must be carried forward — and state that reason in the PR description.
- It does **not** remove the need to tell the user what will be lost. When a change discards existing configuration or data, say so explicitly and describe the manual steps needed to restore equivalent state.

### Practical consequences

- Configuration and settings architecture changes are applied directly; values from an old local setup are re-entered through the UI rather than imported by code.
- Breaking API changes are made in place and recorded in `docs/technical/endpoints.md`; deprecation periods are not introduced by default.
- Before a change that follows this policy, verify there is a known-good revision to return to (`git log`, a tagged or merged commit), so the rollback path actually exists.

See `docs/roadmap/configuration-architecture.md` (section 5) for how this policy is applied to the configuration refactoring.
...
- Migration location: none — this project has no migration modules by policy (see "Development And Rollback Policy")
...
- Authentication / authorization: `app/api/auth_utils.py` (JWT issue/verify, bcrypt), `app/api/deps.py` (`get_current_user`, `require_role`, `require_permission`), `app/api/routers/auth.py` (login, rate limiting); `JWT_SECRET_KEY` env var
...
- Credentials: RTSP URLs in `channels.source` and controller passwords in `controllers.password` (PostgreSQL); infrastructure secrets in `.env`
...
- Database schema evolution is direct-only for development: update `database/postgres/schema.sql` and the current repositories/services in place. Do not introduce DB migration modules, legacy schema adapters, backward-compat database shims, or dual-path support for old database structures. Rollback is a full-project revert to a known-good Git revision — see "Development And Rollback Policy".
