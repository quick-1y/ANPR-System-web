...
- Web UI для операторов и администраторов с системными шрифтами, увеличенной базовой читаемостью и переключаемыми стилями интерфейса
...
Интерфейс поддерживает отдельную настройку **стиля приложения** и **темы оформления**. Стиль `Graphite Minimal` используется по умолчанию и содержит светлую и тёмную темы. Стиль `Aurora` теперь оформлен как отдельный современный визуальный слой с глубиной, glass-поверхностями и отдельными светлой и тёмной палитрами. Настройки сохраняются в `config/settings.yaml` в секции `interface` и доступны в UI: **Настройки → Общие → Интерфейс**.
