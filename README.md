...
Интерфейс поддерживает отдельную настройку **стиля приложения** и **темы оформления**. Стиль `Graphite Minimal` используется по умолчанию и содержит светлую и тёмную темы. Стиль `Aurora` добавлен как изолированная заготовка для будущей палитры и также имеет отдельные токены светлой и тёмной темы. Настройки сохраняются в `config/settings.yaml` в секции `interface` и доступны в UI: **Настройки → Общие → Интерфейс**.
