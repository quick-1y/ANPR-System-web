...
Интерфейс поддерживает отдельную настройку **стиля приложения** и **темы оформления**. Стиль `Graphite Minimal` используется по умолчанию и содержит светлую и тёмную темы. Стиль `Aurora` теперь оформлен как отдельный современный визуальный слой с глубиной, glass-поверхностями и отдельными светлой и тёмной палитрами. Тема и стиль — личное предпочтение пользователя (кнопки в верхней панели, окно «Мои предпочтения»); администратор задаёт значения по умолчанию для всех в **Настройки → Общие → Интерфейс**. Настройки хранятся в PostgreSQL, файла настроек нет.
