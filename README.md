...
| `PUT` | `/api/settings` | Обновить настройки (изменение параметров распознавания номеров перезапускает pipeline) |
...
- CSV/ZIP-экспорт формируется сервером во временном системном каталоге и отдаётся в браузер как файл для скачивания.
