...
Интерфейс поддерживает отдельную настройку **стиля приложения** и **темы оформления**. Стиль `Graphite Minimal` используется по умолчанию и содержит светлую и тёмную темы. Стиль `Aurora` теперь оформлен как отдельный современный визуальный слой с глубиной, glass-поверхностями и отдельными светлой и тёмной палитрами. Тема и стиль — состояние конкретного браузера (`localStorage`), а не учётной записи: кнопка темы в верхней панели и окно «Мои предпочтения». Сервера и синхронизации между рабочими местами нет — выбор не переезжает вместе с пользователем на другой компьютер. Закрепление левой панели — кнопка внутри самой панели, тем же способом (`localStorage`).
...
| ⚙️ | [Конфигурация](docs/technical/configuration.md) | Классы настроек D/A/U/C/L, приоритеты, модели темы и времени, реестр всех ключей |
