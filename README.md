...
Интерфейс поддерживает отдельную настройку **стиля приложения** и **темы оформления**. Стиль `Graphite Minimal` используется по умолчанию и содержит светлую и тёмную темы. Стиль `Aurora` теперь оформлен как отдельный современный визуальный слой с глубиной, glass-поверхностями и отдельными светлой и тёмной палитрами. Тема и стиль — исключительно личное предпочтение пользователя: кнопка темы в верхней панели и окно «Мои предпочтения». Глобального значения по умолчанию нет; закрепление левой панели — кнопка внутри самой панели.
