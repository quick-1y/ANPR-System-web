...
| Метод | Путь | Описание |
|---|---|---|
| `GET` | `/api/settings` | Все глобальные настройки, включая секцию `ui` для стиля приложения, темы, сетки preview и фиксации sidebar |
| `PUT` | `/api/settings` | Обновить настройки; секция `ui` меняет только интерфейс, а изменение параметров распознавания номеров и DSN перезапускает pipeline |
