...
- **Только superadmin** — требует роль `superadmin` (технический аккаунт без строки в `users`, определён `SUPERADMIN_PASSWORD`)
...
- `POST /api/auth/login` — поле `warn_default_password: bool` в ответе: `true` при входе технического superadmin (логин `superadmin`, без строки в `users`), если `SUPERADMIN_PASSWORD` не задан и в силе резервный пароль `1234`; для обычного пользователя БД — `true`, если пароль ни разу не менялся (`password_changed_at IS NULL`); во всех остальных случаях — `false`.
...
Защита от самоблокировки: нельзя деактивировать собственную учётную запись. Отдельной защиты «последнего superadmin» больше нет — superadmin не имеет строки в `users`, эти эндпоинты не могут её достичь, а `role` в `PUT /api/users/{id}` не принимает значение `superadmin`.
