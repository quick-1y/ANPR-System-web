...
Схемы видеоввода, внутреннего pipeline и жизненного цикла событий вынесены в [`docs/diagrams.md`](diagrams.md).
...
Контракты OCR-модели (`32×128`, grayscale, алфавит `0123456789ABCEHKMOPTXY`) и YOLO detector (`bbox_padding_ratio=0.08`, `min_padding_pixels=2`) зафиксированы в `anpr/model_config.py` (порог уверенности детектора — операционная настройка `detection.confidence_threshold` в `app_settings`, по умолчанию 0.5): контракт моделей, потому что менять их безопасно только вместе с проверенными/переобученными моделями. В настройках остаются runtime-параметры OCR-агрегации и per-channel параметры запуска detector.
