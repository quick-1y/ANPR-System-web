...
Глобальные runtime-параметры — пути к моделям, общие пороги detector, логика переподключения, политика retention, логирование, часовой пояс, debug-флаги и список поддерживаемых стран — хранятся в `config/settings.yaml` и управляются через UI настроек или напрямую в файле.

OCR-контракт модели (размер входа `32×128` и алфавит `0123456789ABCEHKMOPTXY`) не является runtime-настройкой и зафиксирован в `anpr/model_config.py`; менять его безопасно только вместе с переобученной/заменённой OCR-моделью.
