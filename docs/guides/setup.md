...
**Также обязательно смените пароль по умолчанию (`superadmin` / `1234`, см. шаг 5) сразу после первого входа.** Он одинаков в каждой установке и указан в этой документации и в тестовых фикстурах — оставленным без изменения он даёт полный доступ (включая бэкап/восстановление БД) любому, кто знает URL установки.
...
Логин по умолчанию: `superadmin` / `1234` — **смените сразу после первого входа** (см. предупреждение на шаге 2)
