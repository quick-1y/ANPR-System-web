...
Интерфейс использует системный стек шрифтов браузера (`Aptos`, `Segoe UI`, `Roboto`, `Noto Sans`, `system-ui`) вместо принудительного Inter. Это уменьшает слипание букв на жирных начертаниях и повышает читаемость на Windows, Linux и мобильных устройствах.

Базовый размер текста увеличен до 16px, а управляющие элементы, таблицы, подписи и служебные бейджи имеют минимальные размеры через CSS-переменные в `app/web/css/styles.css`. Если оператору требуется ещё более крупный текст, рекомендуется дополнительно использовать масштаб страницы браузера 110–125%: layout Web UI сохраняет прокрутку и переносы в основных панелях.
...
Глобальные runtime-параметры — пути к моделям, логика переподключения, политика retention, логирование, часовой пояс, debug-флаги и список поддерживаемых стран — хранятся в `config/settings.yaml` и управляются через UI настроек или напрямую в файле.
