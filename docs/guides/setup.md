...
Глобальные runtime-параметры — пути к моделям, OCR-алфавит и размер входа, общие пороги detector, логика переподключения, политика retention, логирование, часовой пояс, debug-флаги и список поддерживаемых стран — хранятся в `config/settings.yaml` и управляются через UI настроек или напрямую в файле.
