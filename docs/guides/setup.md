...
Глобальные runtime-параметры — пути к моделям, логика переподключения, политика retention, логирование, часовой пояс, настройки интерфейса, debug-флаги и список поддерживаемых стран — хранятся в `config/settings.yaml` и управляются через UI настроек или напрямую в файле.
...
Секция `interface` управляет внешним видом web UI:
...
- `style` — визуальный стиль приложения. Сейчас доступны `graphite-minimal` (дефолтный Graphite Minimal) и `aurora` (отдельный современный стиль с собственной палитрой и layout-надстройками).
- `theme` — тема внутри выбранного стиля: `light` или `dark`. Каждый стиль обязан иметь обе темы.
- `sidebar_locked` — фиксация левой панели в свёрнутом виде.

Оператор меняет стиль и тему в **Настройки → Общие → Интерфейс**; сохранение проходит через `GET/PUT /api/settings`.
