...
Этот документ фиксирует возможный будущий дизайн секции `inference` для ANPR System. Сейчас настройки inference в системе не существуют (файла настроек больше нет), потому что прежние параметры `workers` и `shared_memory` не использовались runtime-кодом. Возвращать её стоит только вместе с реальной реализацией управления ML-инференсом.
...
Настройки inference стоит вводить только если одновременно выполнены условия:

1. ключи объявлены в `config/registry.py` (класс A с `requires_restart` либо класс D) и читаются через `SettingsService` / `EnvConfig`;
2. реестр валидирует все поля;
...
Такой путь даст практическую пользу без преждевременного усложнения runtime и без риска снова получить неиспользуемый ключ в реестре.
